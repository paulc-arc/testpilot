...
Use this skill when GPT-5-mini operates TestPilot `wifi_llapi` normal tests. Normal test means runtime execution through `testpilot run wifi_llapi`; it is not audit mode, repository pytest, schema cleanup, report comparison, or YAML editing.
...
| --- | --- | --- |
| `full-run` | `PYTHONPATH=.:src:plugins/wifi_llapi uv run python -m testpilot.cli run wifi_llapi` | Execute the complete discoverable `wifi_llapi` catalog and produce a report bundle. |
| `by-case` | `PYTHONPATH=.:src:plugins/wifi_llapi uv run python -m testpilot.cli run wifi_llapi --case <CASE_ID>` | Re-run one explicit case, or a small explicit set of cases, using normal runtime behavior. |

Short form examples may be shown as `testpilot run wifi_llapi` and `testpilot run wifi_llapi --case <CASE_ID>`, but the `uv run python -m testpilot.cli ...` form is preferred in this repository.
...
| --- | --- |
| Running pytest and calling it a full run | Use `testpilot run wifi_llapi`. |
