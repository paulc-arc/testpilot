...
def test_release_validation_accepts_matching_tag() -> None:
    result = subprocess.run(
        [sys.executable, "scripts/check_release_version.py", "--tag", "v0.2.0"],
        cwd=ROOT,
        capture_output=True,
        text=True,
    )
    ...
    assert result.returncode == 0, result.stderr
    assert "Validated release tag v0.2.0" in result.stdout
...
def test_release_validation_rejects_mismatched_tag() -> None:
    ...
    assert "does not match expected v0.2.0" in result.stderr
