...
- latest targeted checkpoint: the 6G recovery hot path now short-circuits when `wl -i wl1 bss` reports a missing adapter and defers direct `wl1` hostapd restart to the later DUT reload/bounce path; once `wl1` is present it still re-patches `ocv=0` and re-checks `ocv/socket/bss` together before continuing. Regression after this change: targeted recovery tests `8 passed`, full suite `1645 passed`
...
- latest targeted checkpoint：6G recovery hot path 現在在 `wl -i wl1 bss` 回 `adapter missing` 時會直接 short-circuit，避免誤做 direct `wl1` hostapd restart，並把後續修復交給 DUT reload/bounce path；一旦 `wl1` 存在，仍會重新 patch `ocv=0`、再把 `ocv/socket/bss` 三個訊號一起短窗口驗證後才放行。這批修正的回歸結果為：targeted recovery tests `8 passed`、full suite `1645 passed`
