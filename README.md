...
- latest calibration checkpoint: `D022 HtCapabilities` is now aligned via official rerun `20260412T175538121906`. Stale alias metadata is removed, `source.row` is refreshed from `19` to workbook row `22`, and the durable driver oracle is now the parsed `wl sta_info` `HT caps 0x...` bitmask instead of a brittle token scrape; live evidence exact-closes `HtCapabilities="40MHz,SGI20,SGI40"` with `DriverHt40MHz=1`, `DriverHtSgi20=1`, and `DriverHtSgi40=1`. Targeted D022 guardrails are `1 passed`, overlay compare is now `238 / 420 full matches` / `182 mismatches` / `62 drifts`, actionable workbook-Pass gaps are `153`, `D020` remains a verified fail-shaped mismatch, and the next ready case is `D034`
