...
- latest calibration checkpoint: `D024 LastDataDownlinkRate` is now aligned via official rerun `20260412T172957084134`. Stale alias metadata is removed, `source.row` is refreshed from `21` to workbook row `24`, and the source-backed validator now exact-closes the live getter inside the driver last-tx 100-kbps bucket window (`487400 <= 487400 <= 487500`). Targeted D024 guardrails are `6 passed`, full repo regression stays `1645 passed`, overlay compare is now `236 / 420 full matches` / `184 mismatches` / `62 drifts`, actionable workbook-Pass gaps are `155`, patch-scope true-open set remains zero, and the next ready case is `D025`
...
- latest calibration checkpoint：`D024 LastDataDownlinkRate` 已透過 official rerun `20260412T172957084134` 正式對齊。stale alias metadata 已移除，`source.row` 由 `21` 刷新到 workbook row `24`，而 source-backed validator 現在以 driver last-tx 的 100-kbps bucket window 做判定，live evidence exact-close `487400 <= 487400 <= 487500`。D024 targeted guardrails 為 `6 passed`，full repo regression 維持 `1645 passed`；疊上 D024 rerun 後，`compare-0401` 已來到 `236 / 420 full matches`、`184 mismatches`、`62 drifts`、actionable workbook-Pass gap `155`。`0315~0403` patch-scope true-open set 維持清零；下一個 ready case 是 `D025`
