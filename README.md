...
- latest calibration checkpoint: `D025 LastDataUplinkRate` is now aligned via official rerun `20260412T174551843336`. Stale alias metadata is removed, `source.row` is refreshed from `22` to workbook row `25`, and live evidence exact-closes `LastDataUplinkRate=6000` with `DriverLastUplinkRateRounded=6000`; workbook `H`, public HAL comments, and repeated runtime replays continue to support driver `rate of last rx pkt` as the STA -> AP truth source. Targeted D025 guardrails are `3 passed`, overlay compare is now `237 / 420 full matches` / `183 mismatches` / `62 drifts`, actionable workbook-Pass gaps are `154`, `D020` remains a verified fail-shaped mismatch, and the next ready case is `D022`
...
- latest calibration checkpoint：`D025 LastDataUplinkRate` 已透過 official rerun `20260412T174551843336` 正式對齊。stale alias metadata 已移除，`source.row` 由 `22` 刷新到 workbook row `25`，而 live evidence 也 exact-close `LastDataUplinkRate=6000` 與 `DriverLastUplinkRateRounded=6000`；workbook `H`、public HAL comments 與 repeated runtime replays 仍一致支持 `rate of last rx pkt` 作為 STA -> AP truth source。D025 targeted guardrails 為 `3 passed`；疊上 D024/D025 rerun 後，`compare-0401` 已來到 `237 / 420 full matches`、`183 mismatches`、`62 drifts`、actionable workbook-Pass gap `154`。`D020` 維持已驗證的 fail-shaped mismatch；下一個 ready case 是 `D022`
