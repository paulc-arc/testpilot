**2026-04-15 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 已更新為 `378 / 420 full matches`、`42` 筆 mismatch、`43` 筆 metadata drift。** 最新已提交 closure 改為 `D499 SSID WMM AC_VO Stats WmmBytesReceived`（official rerun `20260415T120604251884`）：這筆 workbook-faithful **Pass alignment closure** 已把舊 source row `366` 退休，改回 workbook row `499` 的 direct `WiFi.SSID.{i}.Stats.WmmBytesReceived.AC_VO?` getter，並補上 explicit `WiFi.SSID.{i}.getSSIDStats()` refresh 與 `wl0/wl1/wl2 wme_counters` `AC_VO` rx-byte cross-check；official rerun 在 tri-band 都 exact-close refresh/direct/driver 一致的 `45322 / 32323 / 31588`，維持 `diagnostic_status=Pass`。這也把 SSID-level `WmmBytesReceived` family 內目前 compare-open 的 `AC_BE/AC_VO` 兩筆 closure shape 收斂乾淨：`D496` exact-close tri-band `0 / 0 / 0`，`D499` exact-close tri-band `45322 / 32323 / 31588`，兩者都證實必須先做 `getSSIDStats()` refresh 才能穩定重讀 direct Stats 值。先前的 localized blockers `D490 Radio Stats WmmFailedBytesSent AC_BE`、`D481 Radio Stats WmmBytesReceived AC_VO`、`D482 Radio Stats WmmBytesSent AC_BE`、`D485 Radio Stats WmmBytesSent AC_VO`、`D454 getRadioStats().FailedRetransCount` 與 `D371 AccessPoint.AssociatedDevice.DisassociationTime` 仍維持原狀；`D355-D357` 仍是 CSI-client placeholder backlog，`D359 AccessPoint.IsolationEnable` 仍暫停在 current single-STA lab shape，strict compare 仍先列出 `D414/D415` AssociatedDevice RRM，但 workbook `G` 已明示需要 dual-STA 802.11k split，所以暫維持 readiness review；systemic active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主，`D211` / `D204` 仍是 parked clarification items，`D182` 仍因 shared 6G baseline path 暫停、`D302` 仍是 source-backed blocker；目前下一個 ready actionable 單案 survey target 轉到 `D502 SSID WMM AC_VI Stats WmmBytesSent`。
