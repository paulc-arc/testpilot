**2026-04-14 superseding note：本檔下半部的大盤分析仍保留 2026-04-13 snapshot；最新 strict compare 已更新為 `329 / 420 full matches`、`91` 筆 mismatch、`58` 筆 metadata drift。** 最新已提交 closure 現在是 `D313 getSSIDStats() RetransCount`（official rerun `20260414T204926308849`）：workbook authority 維持 row `313`，但 stale 的 all-pass results_reference 與寬鬆 regex pass-criteria 已退場；official rerun exact-close tri-band workbook-shaped `Not Supported / Not Supported / Not Supported`，同時保留 live getter evidence `RetransCount=0/0/0`。本輪 targeted D313/ssid-stats runtime guardrails=`51 passed`、command-budget=`1 passed`、full repo regression=`1662 passed`。6G baseline 目前仍保留 `dut_ocv_not_zero` post-verify drift，因此 active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主；`D211` / `D204` 仍是 parked clarification items，next ready non-blocked compare-open case 改為 `D316 getSSIDStats() UnknownProtoPacketsReceived`。
