**2026-04-15 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 已更新為 `375 / 420 full matches`、`45` 筆 mismatch、`43` 筆 metadata drift。** 最新已提交 closure 改為 `D492 Radio Stats WmmFailedBytesSent AC_VI`（official rerun `20260415T112139068980`）：這筆 workbook-faithful **Pass alignment closure** 已把舊 source row `359` 與 `getRadioStats() | grep AC_VI_Stats` replay 退休，改回 workbook row `492` 的 lowercase direct `WiFi.Radio.{i}.Stats.WmmFailedbytesSent.AC_VI?` getters，並用 `wl0/wl1/wl2 wme_counters` 的 `AC_VI` tx failed bytes 做 driver cross-check；official rerun 在 tri-band 都 exact-close `0 / 0 / 0` 並維持 `diagnostic_status=Pass`。雖然 rerun 啟動時再次出現 `serialwrap daemon start failed` warning，但 decoded DUT/STA logs 仍正常落盤，不影響 evidence。同族前一筆 `D491 Radio Stats WmmFailedBytesSent AC_BK` closure 也維持 tri-band exact-close `0 / 0 / 0`；而再前一筆 focused official rerun `20260415T110106382425` 仍把 `D490 Radio Stats WmmFailedBytesSent AC_BE` 明確確認為 localized blocker，而不是 closure：正確 live getter namespace 同樣是 lowercase `WiFi.Radio.{i}.Stats.WmmFailedbytesSent.AC_BE?`（camel-case `WmmFailedBytesSent` 會 object not found），但 tri-band shape 變成 `5G 0=0`、`6G 0 vs 708116`、`2.4G 90=90`，也就是只有 6G direct getter 固定停在 `0`，而 `wl1 wme_counters` `AC_BE` tx failed bytes 穩定為 `708116`；因此 D490 exploratory workbook-faithful rewrite 已回退、不進 commit。先前的 localized blockers `D481 Radio Stats WmmBytesReceived AC_VO`、`D482 Radio Stats WmmBytesSent AC_BE`、`D485 Radio Stats WmmBytesSent AC_VO`、`D454 getRadioStats().FailedRetransCount` 與 `D371 AccessPoint.AssociatedDevice.DisassociationTime` 仍維持原狀；`D355-D357` 仍是 CSI-client placeholder backlog，`D359 AccessPoint.IsolationEnable` 仍暫停在 current single-STA lab shape，strict compare 仍先列出 `D414/D415` AssociatedDevice RRM，但 workbook `G` 已明示需要 dual-STA 802.11k split，所以暫維持 readiness review；systemic active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主，`D211` / `D204` 仍是 parked clarification items，`D182` 仍因 shared 6G baseline path 暫停、`D302` 仍是 source-backed blocker；目前下一個 ready actionable 單案 survey target 轉到 `D493 Radio Stats WmmFailedBytesSent AC_VO`。
