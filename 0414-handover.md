**2026-04-15 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 仍是 `382 / 420 full matches`、`38` 筆 mismatch、`43` 筆 metadata drift。** 最新已提交 closure 仍是 `D507 SSID WMM AC_VO Stats WmmFailedBytesReceived`（official rerun `20260415T125611587722`）：這筆 workbook-faithful **Pass alignment closure** 已把舊 source row `374` 退休，改回 workbook row `507` 的 direct `WiFi.SSID.{i}.Stats.WmmFailedBytesReceived.AC_VO?` getter，並補上 explicit `WiFi.SSID.{i}.getSSIDStats()` refresh 與 `wl0/wl1/wl2 wme_counters` `AC_VO` rx failed-byte cross-check；official rerun 在 tri-band 都 exact-close refresh/direct/driver 一致的 `0 / 0 / 0`，維持 `diagnostic_status=Pass`。緊接著的 focused official rerun `20260415T131457874117` 已確認 `D508 SSID WMM AC_BE Stats WmmFailedBytesSent` 不能 land 成 workbook `Pass / Pass / Pass`：5G 仍 exact-close `0 / 0 / 0`、6G 仍 exact-close `708116 / 708116 / 708116`，但 2.4G 的 `getSSIDStats()` 與 direct getter 都固定回 `0`，而 `wl2 wme_counters` `AC_BE` tx failed bytes 穩定為 `90`，因此目前改列 localized **2.4G zero-getter blocker**，exploratory workbook-faithful rewrite 已回退、不進 commit。到目前為止，SSID-level WMM family 已累積六筆 closure 形狀：`D496` exact-close `WmmBytesReceived.AC_BE` tri-band `0 / 0 / 0`、`D499` exact-close `WmmBytesReceived.AC_VO` tri-band `45322 / 32323 / 31588`、`D502` exact-close `WmmBytesSent.AC_VI` tri-band `0 / 0 / 0`、`D505` exact-close `WmmFailedBytesReceived.AC_BK` tri-band `0 / 0 / 0`、`D506` exact-close `WmmFailedBytesReceived.AC_VI` tri-band `0 / 0 / 0`、`D507` exact-close `WmmFailedBytesReceived.AC_VO` tri-band `0 / 0 / 0`；六者都證實 current SSID stats family 需要 explicit `getSSIDStats()` refresh 才能穩定重讀 direct Stats 值，再用 `wl wme_counters` 做 driver cross-check。既有 localized blockers `D490 Radio Stats WmmFailedBytesSent AC_BE`、`D481 Radio Stats WmmBytesReceived AC_VO`、`D482 Radio Stats WmmBytesSent AC_BE`、`D485 Radio Stats WmmBytesSent AC_VO`、`D454 getRadioStats().FailedRetransCount`、`D371 AccessPoint.AssociatedDevice.DisassociationTime` 之外，現在再新增 `D508`；`D355-D357` 仍是 CSI-client placeholder backlog，`D359 AccessPoint.IsolationEnable` 仍暫停在 current single-STA lab shape，strict compare 仍先列出 `D414/D415` AssociatedDevice RRM，但 workbook `G` 已明示需要 dual-STA 802.11k split，所以暫維持 readiness review；systemic active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主，`D211` / `D204` 仍是 parked clarification items，`D182` 仍因 shared 6G baseline path 暫停、`D302` 仍是 source-backed blocker；目前下一個 ready actionable 單案 survey target 轉到 `D510 SSID WMM AC_VI Stats WmmFailedBytesSent`。
