**2026-04-14 superseding note：本檔下半部的大盤分析仍保留 2026-04-13 snapshot；最新 strict compare 已更新為 `331 / 420 full matches`、`89` 筆 mismatch、`58` 筆 metadata drift。** 最新已提交 closure 現在是 `D327 ErrorsReceived / SSID stats`（official rerun `20260414T210918023652`）：workbook authority 維持 row `327`，但 stale 的 all-pass results_reference 已退場；official rerun exact-close tri-band workbook-shaped `Skip / Skip / Skip`，同時保留 live direct/getSSIDStats()/driver cross-check evidence `ErrorsReceived=0/0/0`。本輪 targeted D327/direct-stats runtime guardrails=`1 passed`、command-budget=`1 passed`、full repo regression=`1662 passed`。6G baseline 目前仍保留 `dut_ocv_not_zero` post-verify drift，因此 active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主；`D211` / `D204` 仍是 parked clarification items，next ready non-blocked compare-open case 改為 `D337 UnknownProtoPacketsReceived / SSID stats`。
