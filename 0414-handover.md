**2026-04-14 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 已更新為 `336 / 420 full matches`、`84` 筆 mismatch、`58` 筆 metadata drift。** 最新已提交 closure 現在是 `D336 UnicastPacketsSent / SSID stats`（official rerun `20260414T231646005774`）：stale 的分步 direct/getSSIDStats()/driver 採樣已由 same-window snapshot rewrite 取代，official rerun exact-close tri-band workbook-shaped `Pass / Pass / Pass`，同時保留 live cross-check evidence `UnicastPacketsSent=3621/2595/3997` 與 `diagnostic_status=Pass`。本輪 targeted D336/direct-stats runtime guardrails + command-budget=`2 passed`、full repo regression=`1662 passed`。6G baseline 目前仍保留 `dut_ocv_not_zero` post-verify drift，因此 active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主；`D211` / `D204` 仍是 parked clarification items，next ready non-blocked compare-open case 改為 `D296 StartACS`。
