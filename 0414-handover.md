**2026-04-15 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 已更新為 `359 / 420 full matches`、`61` 筆 mismatch、`47` 筆 metadata drift。** 最新已提交 closure 現在是 `D436 AccessPoint.Security.OWETransitionInterface`（official rerun `20260415T054920871826`）：workbook authority 已刷新為 row `436`，舊 row `438` getter shell 已改寫回 workbook `WiFi.AccessPoint.{i}.Security.` / `OWETransitionInterface` setter/getter + hostapd replay。current rerun 會在 AP1/AP3/AP5 穩定讀回 `DEFAULT_WL1_1 / DEFAULT_WL0_1 / DEFAULT_WL0_1`；wl1 / wl2 仍維持 `owe_transition_ifname=ABSENT`，但 wl0 於 workbook 5G `AP1.ModeEnabled=OWE` / `AP2.ModeEnabled=None` pair step 期間會投影 `HostapdOweTransitionIfname5g=wl1`，因此此案以 workbook-faithful `Fail / Fail / Fail` / `diagnostic_status=FailTest` 關閉 compare row。targeted D436/runtime + budget guardrails 已通過、full repo regression=`1659 passed`。`D371 AccessPoint.AssociatedDevice.DisassociationTime` 仍維持 localized blocker：focused survey runs `20260415T014146461381` / `20260415T015629548681` / `20260415T020725267608` 先後暴露 24G `assoclist` 殘留且 `DisassociationTime` 不變、5G `assoclist` 殘留，以及 6G `step11_6g_post_assoc` serialwrap timeout，因此該 rewrite 仍回退、不進 commit。`D355-D357` 仍是需要 CSI client setup 的 placeholder backlog；`D359 AccessPoint.IsolationEnable` 也仍暫停，因 workbook 明確需要 two-station isolation ping，但 current lab/testbed flow 只有標準 single-STA path。systemic active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主，`D211` / `D204` 仍是 parked clarification items，`D182` 仍因 shared 6G baseline path 暫停、`D302` 仍是 source-backed blocker；strict compare 仍先列出 `D414/D415` AssociatedDevice RRM cases，但這兩筆 workbook `G` 已明示需要一台 802.11k enable、另一台 disable 的 dual-STA shape，所以暫維持 readiness review；目前下一個 ready actionable 單案 survey target 轉到 `D437 AccessPoint.Security.SAEPassphrase`。
