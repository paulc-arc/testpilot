**2026-04-15 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 已更新為 `360 / 420 full matches`、`60` 筆 mismatch、`47` 筆 metadata drift。** 最新已完成 closure 現在是 `D437 AccessPoint.Security.SAEPassphrase`（official rerun `20260415T061648225671`）：workbook authority 已刷新為 row `437`，舊 row `439` generic getter shell 已改寫回 workbook `WiFi.AccessPoint.{i}.Security.` / `SAEPassphrase` setter/getter + hostapd `sae_password` replay。current rerun exact-close `Pass / Pass / Pass`：baseline 維持 5G/2.4G getter `password` + hostapd `ABSENT`、6G getter / hostapd `00000000`；wildcard `ModeEnabled=WPA3-Personal` + `SAEPassphrase=1234567890` 後，AP1/AP3/AP5 getter 與 wl0/wl1/wl2 hostapd `sae_password=` 全部收斂到 `1234567890`；restore 也穩定回到 5G/2.4G `WPA2-Personal + password + no sae_password`、6G `WPA3-Personal + quoted 00000000`。quoted `SAEPassphrase="00000000"` 的 setup baseline 也已經透過 plugin `sync_psk` fix 正確同步成 `KeyPassPhrase="00000000"`，因此 targeted D437/runtime + budget guardrails 已通過、full repo regression=`1660 passed`。`D371 AccessPoint.AssociatedDevice.DisassociationTime` 仍維持 localized blocker：focused survey runs `20260415T014146461381` / `20260415T015629548681` / `20260415T020725267608` 先後暴露 24G `assoclist` 殘留且 `DisassociationTime` 不變、5G `assoclist` 殘留，以及 6G `step11_6g_post_assoc` serialwrap timeout，因此該 rewrite 仍回退、不進 commit。`D355-D357` 仍是需要 CSI client setup 的 placeholder backlog；`D359 AccessPoint.IsolationEnable` 也仍暫停，因 workbook 明確需要 two-station isolation ping，但 current lab/testbed flow 只有標準 single-STA path。systemic active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主，`D211` / `D204` 仍是 parked clarification items，`D182` 仍因 shared 6G baseline path 暫停、`D302` 仍是 source-backed blocker；strict compare 仍先列出 `D414/D415` AssociatedDevice RRM cases，但這兩筆 workbook `G` 已明示需要一台 802.11k enable、另一台 disable 的 dual-STA shape，所以暫維持 readiness review；目前下一個 ready actionable 單案 survey target 轉到 `D438 AccessPoint.Security.TransitionDisable`。
