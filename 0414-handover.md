**2026-04-15 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 仍為 `362 / 420 full matches`、`58` 筆 mismatch、`45` 筆 metadata drift。** 最新已完成 closure 現在是 `D464 NonSRGOffsetValid`（official rerun `20260415T080003753294`）：這筆是 **fail-shaped alignment closure**，不是 pass-row flip。workbook authority 已刷新為 row `464`，舊 row `341` 誤指到 `WiFi.wps_DefParam.FriendlyName` 的 metadata 已改回 workbook `WiFi.Radio.{i}.IEEE80211ax.` / `NonSRGOffsetValid` getter replay；但 workbook answer `R/S/T` 本身就是 `Fail / Fail / Fail`，因此 landed case 仍保留 fail-shaped `results_reference`，current rerun 也確實 exact-close `Fail / Fail / Fail`，同時 diagnostic layer 仍為 `Pass`，因 tri-band getter 都穩定回 `0 / 0 / 0`。targeted D464/runtime tests 已通過；這筆 closure 讓 strict compare metadata drift 進一步降到 `45`。同一批 focused workbook-faithful survey 也確認 `D454 getRadioStats().FailedRetransCount` 目前仍是 localized blocker：official rerun `20260415T064937785938` 在 5G/2.4G 仍可 exact-close `FailedRetransCount=100/946` 對 `wl0/wl2 counters txfail=100/946`，但 6G 固定漂移成 `FailedRetransCount=0` vs `wl1 counters txfail=740`，因此 exploratory rewrite 已回退、不進 commit。`D371 AccessPoint.AssociatedDevice.DisassociationTime` 仍維持 localized blocker：focused survey runs `20260415T014146461381` / `20260415T015629548681` / `20260415T020725267608` 先後暴露 24G `assoclist` 殘留且 `DisassociationTime` 不變、5G `assoclist` 殘留，以及 6G `step11_6g_post_assoc` serialwrap timeout，因此該 rewrite 仍回退、不進 commit。`D355-D357` 仍是需要 CSI client setup 的 placeholder backlog；`D359 AccessPoint.IsolationEnable` 也仍暫停，因 workbook 明確需要 two-station isolation ping，但 current lab/testbed flow 只有標準 single-STA path。systemic active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主，`D211` / `D204` 仍是 parked clarification items，`D182` 仍因 shared 6G baseline path 暫停、`D302` 仍是 source-backed blocker；strict compare 仍先列出 `D414/D415` AssociatedDevice RRM cases，但這兩筆 workbook `G` 已明示需要一台 802.11k enable、另一台 disable 的 dual-STA shape，所以暫維持 readiness review；目前下一個 ready actionable 單案 survey target 轉到 `D474 channel_radio_37`。
