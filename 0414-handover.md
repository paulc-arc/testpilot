**2026-04-15 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 仍維持 `367 / 420 full matches`、`53` 筆 mismatch、`43` 筆 metadata drift。** 最新已完成 closure 仍是 `D480 Radio Stats WmmBytesReceived AC_VI`（official rerun `20260415T092125550117`）：這筆是 workbook-faithful **Pass alignment closure**，舊 source row `347` 與 `getRadioStats() | grep AC_VI_Stats` replay 已退休，landed case 現在以 workbook row `480` 的 tri-band direct `WiFi.Radio.{i}.Stats.WmmBytesReceived.AC_VI?` getters 對位 `wl0/wl1/wl2 wme_counters` `AC_VI` RX bytes，official rerun exact-close `0 / 0 / 0`，並維持 `diagnostic_status=Pass`；targeted D480/runtime tests、command-budget guardrail 與 full repo regression（`1660 passed`）都已通過。緊接著的 `D481 Radio Stats WmmBytesReceived AC_VO` focused workbook-faithful rerun `20260415T093205719889` 則確認新的 localized blocker：5G/2.4G direct getter 仍可 exact-close `45322 / 31588` 對 `wl0/wl2 wme_counters`，但 6G direct getter 固定停在 `0`，driver `wl1 wme_counters` `AC_VO` RX bytes 卻穩定為 `32323`；follow-up serialwrap probe 也確認 `ubus-cli "WiFi.Radio.2.Stats.WmmBytesReceived.AC_VO?"` 在 `getRadioStats()` refresh 前後都維持 `0`，而 `ubus-cli "WiFi.Radio.2.getRadioStats()" | grep AC_VO_Stats` 仍是空輸出，因此 exploratory rewrite 已回退、不進 commit，這筆目前暫列 localized blocker。先前 focused workbook-faithful survey 也仍確認 `D454 getRadioStats().FailedRetransCount` 是 localized blocker：official rerun `20260415T064937785938` 在 5G/2.4G 仍可 exact-close `FailedRetransCount=100/946` 對 `wl0/wl2 counters txfail=100/946`，但 6G 固定漂移成 `FailedRetransCount=0` vs `wl1 counters txfail=740`；`D371 AccessPoint.AssociatedDevice.DisassociationTime` 也仍維持 localized blocker。`D355-D357` 仍是需要 CSI client setup 的 placeholder backlog；`D359 AccessPoint.IsolationEnable` 仍暫停，因 workbook 明確需要 two-station isolation ping，但 current lab/testbed flow 只有標準 single-STA path。systemic active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主，`D211` / `D204` 仍是 parked clarification items，`D182` 仍因 shared 6G baseline path 暫停、`D302` 仍是 source-backed blocker；strict compare 仍先列出 `D414/D415` AssociatedDevice RRM cases，但這兩筆 workbook `G` 已明示需要一台 802.11k enable、另一台 disable 的 dual-STA shape，所以暫維持 readiness review；目前下一個 ready actionable 單案 survey target 轉到 `D482 Radio Stats WmmBytesSent AC_BE`。
