**2026-04-15 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 已更新為 `344 / 420 full matches`、`76` 筆 mismatch、`57` 筆 metadata drift。** 最新已提交 closure 現在是 `D377 Radio.MaxBitRate`（official rerun `20260415T023436252245`）：stale 的 row `280` 已回到 workbook row `377`，live getter 仍穩定回 `MaxBitRate=0/0/0`，而 landed case 現在依 workbook authority 投影 `Not Supported / Not Supported / Not Supported`、同時保留 `diagnostic_status=Pass` 的 getter evidence；targeted D377/runtime + radio-getter guardrails 已通過、full repo regression=`1661 passed`。`D371 AccessPoint.AssociatedDevice.DisassociationTime` 則已在 focused survey runs `20260415T014146461381` / `20260415T015629548681` / `20260415T020725267608` 被確認為 blocker：first replay 先暴露 24G `assoclist` 殘留且 `DisassociationTime` 不變，後續 driver-level detach trials 又分別出現 5G `assoclist` 殘留與 6G `step11_6g_post_assoc` serialwrap timeout，因此該 rewrite 已回退、不進 commit。`D355-D357` 仍是需要 CSI client setup 的 placeholder backlog；`D359 AccessPoint.IsolationEnable` 也仍暫停，因 workbook 明確需要 two-station isolation ping，但 current lab/testbed flow 只有標準 single-STA path。systemic active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主，`D211` / `D204` 仍是 parked clarification items，`D182` 仍因 shared 6G baseline path 暫停、`D302` 仍是 source-backed blocker，因此 next ready non-blocked compare-open case 改為 `D379 Radio.MCS`。
