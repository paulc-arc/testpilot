**2026-04-15 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 已更新為 `384 / 420 full matches`、`36` 筆 mismatch、`43` 筆 metadata drift。** 最新已提交 closure 已前進到 `D512 SSID WMM AC_BE Stats WmmFailedReceived`（official rerun `20260415T134510589533`）：這筆 workbook-faithful **Pass alignment closure** 已把舊 source row `379` 退休，改回 workbook row `512` 的 direct `WiFi.SSID.{i}.Stats.WmmFailedReceived.AC_BE?` getter，並補上 explicit `WiFi.SSID.{i}.getSSIDStats()` refresh 與 `wl0/wl1/wl2 wme_counters` `AC_BE` rx failed-frame cross-check；official rerun 在 tri-band 都 exact-close refresh/direct/driver 一致的 `0 / 0 / 0`，維持 `diagnostic_status=Pass`。較早的 focused official rerun `20260415T131457874117` 也已確認 `D508 SSID WMM AC_BE Stats WmmFailedBytesSent` 不能 land 成 workbook `Pass / Pass / Pass`：5G 仍 exact-close `0 / 0 / 0`、6G 仍 exact-close `708116 / 708116 / 708116`，但 2.4G 的 `getSSIDStats()` 與 direct getter 都固定回 `0`，而 `wl2 wme_counters` `AC_BE` tx failed bytes 穩定為 `90`，因此目前仍列 localized **2.4G zero-getter blocker**。到目前為止，SSID-level WMM family 已累積八筆 closure 形狀：`D496`、`D499`、`D502`、`D505`、`D506`、`D507`、`D510`、`D512`；八者都證實 current SSID stats family 需要 explicit `getSSIDStats()` refresh 才能穩定重讀 direct Stats 值，再用 `wl wme_counters` 做 driver cross-check。既有 localized blockers `D490 Radio Stats WmmFailedBytesSent AC_BE`、`D481 Radio Stats WmmBytesReceived AC_VO`、`D482 Radio Stats WmmBytesSent AC_BE`、`D485 Radio Stats WmmBytesSent AC_VO`、`D454 getRadioStats().FailedRetransCount`、`D371 AccessPoint.AssociatedDevice.DisassociationTime` 之外，`D508` 仍維持 blocker；`D355-D357` 仍是 CSI-client placeholder backlog，`D359 AccessPoint.IsolationEnable` 仍暫停在 current single-STA lab shape，strict compare 仍先列出 `D414/D415` AssociatedDevice RRM，但 workbook `G` 已明示需要 dual-STA 802.11k split，所以暫維持 readiness review；systemic active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主，`D211` / `D204` 仍是 parked clarification items，`D182` 仍因 shared 6G baseline path 暫停、`D302` 仍是 source-backed blocker。最新 verification 結果為 targeted runtime/budget guardrails `1251 passed`、full repo regression `1660 passed`；目前下一個 ready actionable 單案 survey target 轉到 `D513 SSID WMM AC_BK Stats WmmFailedReceived`。
