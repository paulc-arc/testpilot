**2026-04-14 superseding note：本檔下半部的大盤分析仍保留 2026-04-13 snapshot；最新 strict compare 已更新為 `335 / 420 full matches`、`85` 筆 mismatch、`58` 筆 metadata drift。** 最新已提交 closure 現在是 `D334 RetransCount / SSID stats`（official rerun `20260414T222507260531`）：stale 的 all-pass results_reference 已退場；official rerun exact-close tri-band workbook-shaped `Fail / Fail / Fail`，同時保留 live direct/getSSIDStats()/driver cross-check evidence `RetransCount=0/0/0` 與 `diagnostic_status=Pass`。本輪 targeted D334/direct-stats runtime guardrails + command-budget=`2 passed`、full repo regression=`1662 passed`。6G baseline 目前仍保留 `dut_ocv_not_zero` post-verify drift，因此 active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主；`D211` / `D204` 仍是 parked clarification items，next ready non-blocked compare-open case 改為 `D336 UnicastPacketsSent / SSID stats`。
