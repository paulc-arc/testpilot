**2026-04-15 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 已更新為 `367 / 420 full matches`、`53` 筆 mismatch、`43` 筆 metadata drift。** 最新已完成 closure 現在是 `D480 Radio Stats WmmBytesReceived AC_VI`（official rerun `20260415T092125550117`）：這筆是 workbook-faithful **Pass alignment closure**。雖然 object/api 原本已經對到 workbook `WiFi.Radio.{i}.Stats.WmmBytesReceived.` / `AC_VI`，但舊 source row `347` 與 `getRadioStats() | grep AC_VI_Stats` replay 仍是 stale authored shape；landed case 現在已改回 workbook row `480` 的 tri-band direct `WiFi.Radio.{i}.Stats.WmmBytesReceived.AC_VI?` getters，並用 `wl0/wl1/wl2 wme_counters` 的 `AC_VI` RX bytes 做 driver cross-check，official rerun 一致 exact-close `0 / 0 / 0`，因此 final report 明確保留 workbook-shaped `Pass / Pass / Pass` 並維持 `diagnostic_status=Pass`。targeted D480/runtime tests、command-budget guardrail 與 full repo regression（`1660 passed`）已通過；這筆 closure 讓 strict compare 再前進到 `367 / 420`，mismatch 降到 `53`、metadata drift 維持 `43`。同一批 focused workbook-faithful survey 仍確認 `D454 getRadioStats().FailedRetransCount` 是 localized blocker：official rerun `20260415T064937785938` 在 5G/2.4G 仍可 exact-close `FailedRetransCount=100/946` 對 `wl0/wl2 counters txfail=100/946`，但 6G 固定漂移成 `FailedRetransCount=0` vs `wl1 counters txfail=740`，因此 exploratory rewrite 已回退、不進 commit。`D371 AccessPoint.AssociatedDevice.DisassociationTime` 仍維持 localized blocker：focused survey runs `20260415T014146461381` / `20260415T015629548681` / `20260415T020725267608` 先後暴露 24G `assoclist` 殘留且 `DisassociationTime` 不變、5G `assoclist` 殘留，以及 6G `step11_6g_post_assoc` serialwrap timeout，因此該 rewrite 仍回退、不進 commit。`D355-D357` 仍是需要 CSI client setup 的 placeholder backlog；`D359 AccessPoint.IsolationEnable` 也仍暫停，因 workbook 明確需要 two-station isolation ping，但 current lab/testbed flow 只有標準 single-STA path。systemic active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主，`D211` / `D204` 仍是 parked clarification items，`D182` 仍因 shared 6G baseline path 暫停、`D302` 仍是 source-backed blocker；strict compare 仍先列出 `D414/D415` AssociatedDevice RRM cases，但這兩筆 workbook `G` 已明示需要一台 802.11k enable、另一台 disable 的 dual-STA shape，所以暫維持 readiness review；目前下一個 ready actionable 單案 survey target 轉到 `D481 Radio Stats WmmBytesReceived AC_VO`。
