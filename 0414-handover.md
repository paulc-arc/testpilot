**2026-04-15 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 已更新為 `350 / 420 full matches`、`70` 筆 mismatch、`55` 筆 metadata drift。** 最新已提交 closure 現在是 `D397 getRadioStats().ErrorsSent`（official rerun `20260415T032303445635`）：stale 的 row `292` 已回到 workbook row `397`，`source.object/api` 也已對齊 workbook `WiFi.Radio.{i}.Stats.` / `ErrorsSent`，而 landed case 保留 tri-band `getRadioStats()` live evidence（`ErrorsSent=2/0/2` on current rerun）並回寫 workbook authority `Pass / Pass / Pass`、同時保留 `diagnostic_status=Pass`；targeted D397/runtime + radio-stats guardrails 已通過、full repo regression=`1667 passed`。`D371 AccessPoint.AssociatedDevice.DisassociationTime` 仍維持 localized blocker：focused survey runs `20260415T014146461381` / `20260415T015629548681` / `20260415T020725267608` 先後暴露 24G `assoclist` 殘留且 `DisassociationTime` 不變、5G `assoclist` 殘留，以及 6G `step11_6g_post_assoc` serialwrap timeout，因此該 rewrite 仍回退、不進 commit。`D355-D357` 仍是需要 CSI client setup 的 placeholder backlog；`D359 AccessPoint.IsolationEnable` 也仍暫停，因 workbook 明確需要 two-station isolation ping，但 current lab/testbed flow 只有標準 single-STA path。systemic active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主，`D211` / `D204` 仍是 parked clarification items，`D182` 仍因 shared 6G baseline path 暫停、`D302` 仍是 source-backed blocker；目前下一個 compare-open cluster 轉到 `D414/D415` AssociatedDevice RRM cases，但 workbook `G` 已明示需要一台 802.11k enable、另一台 disable 的 dual-STA shape，因此進下一輪前要先做 readiness review。
