**2026-04-15 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 已更新為 `343 / 420 full matches`、`77` 筆 mismatch、`57` 筆 metadata drift。** 最新已提交 closure 現在是 `D370 AccessPoint.AssociatedDevice.Active`（official rerun `20260415T011605260076`）：stale 的 row `372` 已回到 workbook row `370`，而 case shape 也從舊的 5G-only getter 提升成 tri-band AssociatedDevice live check；live evidence 目前穩定回 `Active=1/1/1`，且三個 band 的 `wl assoclist` 都能對上同一個 `AssociatedDevice.1` STA MAC，因此 landed case 現在明確投影 workbook-shaped `Pass / Pass / Pass`、同時保留 `diagnostic_status=Pass` 的 getter evidence。本輪 targeted D370/runtime + assocdev guardrails 已通過、full repo regression=`1660 passed`。`D355-D357` 仍是需要 CSI client setup 的 placeholder backlog；`D359 AccessPoint.IsolationEnable` 也已暫停，因 workbook 明確需要 two-station isolation ping，但 current lab/testbed flow 只有標準 single-STA path。6G baseline 目前仍保留 `dut_ocv_not_zero` post-verify drift，因此 active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主；`D211` / `D204` 仍是 parked clarification items，`D182` 仍因 shared 6G baseline path 暫停、`D302` 仍是 source-backed blocker，因此 next ready non-blocked compare-open case 改為 `D371 AccessPoint.AssociatedDevice.DisassociationTime`。
