**2026-04-15 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 仍是 `373 / 420 full matches`、`47` 筆 mismatch、`43` 筆 metadata drift。** 最新已提交 closure 仍是 `D489 Radio Stats WmmFailedBytesReceived AC_VO`（official rerun `20260415T105002687631`）：這筆 workbook-faithful **Pass alignment closure** 仍維持 tri-band direct `WiFi.Radio.{i}.Stats.WmmFailedBytesReceived.AC_VO?` getters 對 `wl0/wl1/wl2 wme_counters` `AC_VO` rx failed bytes 的 exact-close `0 / 0 / 0`；focused serialwrap preprobe 雖曾同時看見 `AC_VO` traffic bytes 本身為非零（5G/6G/2.4G `60420 / 43680 / 42351`），但 row `489` 真正有權威性的仍是 `failed bytes`，因此這筆不是 D481-style blocker。後續 focused official rerun `20260415T110106382425` 已把 `D490 Radio Stats WmmFailedBytesSent AC_BE` 明確確認為新的 localized blocker，而不是 closure：survey 先釘出正確 live getter namespace 是 lowercase `WiFi.Radio.{i}.Stats.WmmFailedbytesSent.AC_BE?`（camel-case `WmmFailedBytesSent` 會 object not found），rerun 也證實 tri-band shape 是 `5G 0=0`、`6G 0 vs 708116`、`2.4G 90=90`，也就是 6G direct getter 固定停在 `0`，但 `wl1 wme_counters` `AC_BE` tx failed bytes 穩定為 `708116`；因此 exploratory workbook-faithful rewrite 已回退，不進 commit，strict compare 也維持不變。先前的 localized blockers `D481 Radio Stats WmmBytesReceived AC_VO`、`D482 Radio Stats WmmBytesSent AC_BE`、`D485 Radio Stats WmmBytesSent AC_VO`、`D454 getRadioStats().FailedRetransCount` 與 `D371 AccessPoint.AssociatedDevice.DisassociationTime` 仍維持原狀；`D355-D357` 仍是 CSI-client placeholder backlog，`D359 AccessPoint.IsolationEnable` 仍暫停在 current single-STA lab shape，strict compare 仍先列出 `D414/D415` AssociatedDevice RRM，但 workbook `G` 已明示需要 dual-STA 802.11k split，所以暫維持 readiness review；systemic active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主，`D211` / `D204` 仍是 parked clarification items，`D182` 仍因 shared 6G baseline path 暫停、`D302` 仍是 source-backed blocker；目前下一個 ready actionable 單案 survey target 轉到 `D491 Radio Stats WmmFailedBytesSent AC_BK`。
