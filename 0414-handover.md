**2026-04-15 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 已更新為 `342 / 420 full matches`、`78` 筆 mismatch、`57` 筆 metadata drift。** 最新已提交 closure 現在是 `D367 IEEE80211ax.SRGOBSSPDMaxOffset`（official rerun `20260415T004735420726`）：stale 的 row `369` 已回到 workbook row `367`，而 live getter 目前穩定回 `SRGOBSSPDMaxOffset=62/62/62`，因此 landed case 現在明確投影 workbook-shaped `Pass / Pass / Pass`、同時保留 `diagnostic_status=Pass` 的 getter evidence。本輪 targeted D367/runtime + getter-batch guardrails=`192 passed`、full repo regression=`1662 passed`。`D355-D357` 仍是需要 CSI client setup 的 placeholder backlog；`D359 AccessPoint.IsolationEnable` 也已暫停，因 workbook 明確需要 two-station isolation ping，但 current lab/testbed flow 只有標準 single-STA path。6G baseline 目前仍保留 `dut_ocv_not_zero` post-verify drift，因此 active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主；`D211` / `D204` 仍是 parked clarification items，`D182` 仍因 shared 6G baseline path 暫停、`D302` 仍是 source-backed blocker，因此 next ready non-blocked compare-open case 改為 `D370 AccessPoint.AssociatedDevice.Active`。
