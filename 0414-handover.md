**2026-04-14 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 已更新為 `338 / 420 full matches`、`82` 筆 mismatch、`58` 筆 metadata drift。** 最新已提交 closure 現在是 `D297 StartAutoChannelSelection`（official rerun `20260414T235120551775`）：stale 的 row `222` 與 bare `startAutoChannelSelection()` replay 已由 workbook note V 明示的 ACS 前置條件取代——每個 band 先 `AutoChannelEnable=1`、再 `startAutoChannelSelection()`，並保留 `iw dev wl0/wl1/wl2 info` evidence；official rerun exact-close tri-band workbook-shaped `Pass / Pass / Pass`，同時保留 live evidence `AutoChannelEnable=1`、`startAutoChannelSelection() returned` 與 readable `iw dev` radio info。本輪 targeted action-method runtime guardrails + command-budget=`16 passed`、full repo regression=`1662 passed`。6G baseline 目前仍保留 `dut_ocv_not_zero` post-verify drift，因此 active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主；`D211` / `D204` 仍是 parked clarification items，`D182` 仍因 shared 6G baseline path 暫停、`D302` 仍是 source-backed blocker，因此 next ready non-blocked compare-open case 改為 `D354 Radio.Enable`。
