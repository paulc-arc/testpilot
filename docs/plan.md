...
7. `wifi_llapi` 目前已固化 151 筆 trusted/calibrated official cases，尚餘 264 筆待校正；目前明確 blockers 為 `D037 OperatingStandard`、`D054 Tx_RetransmissionsFailed`、`D055 TxBytes`，但目前先不插回主 queue，等其餘 sequential 校正收斂後再回頭處理。
...
9. reboot 後的 default lab baseline 已重建並落成文件：5G `testpilot5G` / 2.4G `testpilot2G` 使用 `WPA2-Personal + 00000000`，6G `testpilot6G` 固定使用 `WPA3-Personal + key_mgmt=SAE + 00000000`；`D091 PreSharedKey` 已在這組 baseline 下完成 AP-only multiband checkpoint（workbook row 83；5G/2.4G setter+readback pass，6G WPA3-only → Not Supported），主 sequence 的下一個 ready single-case 入口為 `D092 RekeyingInterval`。
