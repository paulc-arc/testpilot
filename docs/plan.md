...
7. `wifi_llapi` 目前已固化 132 筆 trusted/calibrated cases，尚餘 283 筆待校正；目前明確 blockers 為 `D037 OperatingStandard`、`D054 Tx_RetransmissionsFailed`、`D055 TxBytes`。
8. repo-only 校正 handoff 已同步到 `docs/audit-todo.md` 與 `plugins/wifi_llapi/reports/audit-report-260313-185447.md`；目前 handoff 已補記 `D060 UNIIBandsCapabilities`、`D061 UplinkBandwidth`、`D062 UplinkMCS`、`D063 UplinkShortGuard`、`D064 VendorOUI`、`D065 VhtCapabilities`、`D066 APBridgeDisable`、`D067 BridgeInterface`、`D068 DiscoveryMethodEnabled (FILS)`、`D069 DiscoveryMethodEnabled (UPR)`、`D070 DiscoveryMethodEnabled (RNR)`、`D072 Enable`、`D073 FTOverDSEnable` 的 committed checkpoint 與 `D185 TPCMode` targeted source/live fail-shaped checkpoint。
9. reboot 後的 default lab baseline 已重建並落成文件：5G `testpilot5G` / 2.4G `testpilot2G` 使用 `WPA2-Personal + 00000000`，6G `testpilot6G` 固定使用 `WPA3-Personal + key_mgmt=SAE + 00000000`；`D073 FTOverDSEnable` 已在這組 baseline 下完成 AP-only multiband `To be tested` checkpoint（AP1/AP3/AP5 都需要先滿足 `IEEE80211r.Enabled=1` 與 `MobilityDomain=4660`，northbound getter 會維持十進位 `4660`，hostapd 會存成 `mobility_domain=3412`，而 `FTOverDSEnable` / hostapd `ft_over_ds` 都可在 `0 -> 1 -> 0` 間乾淨收斂；最後已恢復 `Enabled=0`、`MobilityDomain=0`），下一個 ready single-case 入口改為 `D074 MobilityDomain`。
