...
7. `wifi_llapi` 目前已固化 141 筆 trusted/calibrated official cases，尚餘 274 筆待校正；目前明確 blockers 為 `D037 OperatingStandard`、`D054 Tx_RetransmissionsFailed`、`D055 TxBytes`，但目前先不插回主 queue，等其餘 sequential 校正收斂後再回頭處理。
...
9. reboot 後的 default lab baseline 已重建並落成文件：5G `testpilot5G` / 2.4G `testpilot2G` 使用 `WPA2-Personal + 00000000`，6G `testpilot6G` 固定使用 `WPA3-Personal + key_mgmt=SAE + 00000000`；`D081 Mode` 已在這組 baseline 下完成 AP-only multiband `Fail` checkpoint（workbook row 73；AP1 baseline 是 `BlackList` + `deny_mac_file=/tmp/hostap_wl0.acl`，AP3/AP5 baseline 都已是 `Off` 且 hostapd 無 ACL line，但 `ubus-cli WiFi.AccessPoint.{1|3|5}.MACFiltering.Mode=Off` 在三個 band 都仍回 `ERROR: ... invalid value`，並留下不變的 getter / hostapd ACL 狀態），主 sequence 的下一個 ready single-case 入口為 `D082 MaxAssociatedDevices`。
