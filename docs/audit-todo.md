...
- `D437 AccessPoint.Security.SAEPassphrase` is now aligned via official rerun `20260415T061648225671`
- workbook authority is row `437`; the old row `439` generic getter shell is now replaced by a workbook-faithful AP-only `ModeEnabled=WPA3-Personal` + wildcard `SAEPassphrase=1234567890` + hostapd `sae_password` replay on `WiFi.AccessPoint.{i}.Security.`
- the rerun exact-closes `Pass / Pass / Pass`: baseline stays `5G/2.4G = WPA2-Personal + getter password + hostapd ABSENT`, `6G = WPA3-Personal + getter/hostapd 00000000`, the workbook replay converges AP1/AP3/AP5 getter and wl0/wl1/wl2 hostapd `sae_password=` to `1234567890`, and restore returns 5G/2.4G to `WPA2-Personal + password + no sae_password` plus 6G to `WPA3-Personal + quoted 00000000`; final status is `diagnostic_status=Pass`
...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed closure is now `D437 AccessPoint.Security.SAEPassphrase`
- targeted D437/runtime + budget guardrails passed; full repo regression is now `1660 passed`; compare is now `360 / 420 full matches` / `60 mismatches` / `47 metadata drifts`, `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable survey target now moves to `D438 AccessPoint.Security.TransitionDisable`
