...
- Progress record:
  - latest aligned case: `D025 LastDataUplinkRate` via run `20260412T174551843336`
  - latest blocked case: `none in the D025 closure checkpoint; next ready workbook-Pass case is D022`
  - latest compare summary: `237 / 420` full matches, `183` mismatches, `62` metadata drifts (overlay of full run `20260412T113008433351` plus D024 rerun `20260412T172957084134` plus D025 rerun `20260412T174551843336`)
  ...
  - `D026` remains the next stale-row follow-up after the current D022 pass-gap closure: workbook authority is row `26`, and the source evidence still points to DUT `wl -i wl0 sta_info $STA_MAC` `link bandwidth = XX MHZ` plus `WiFi.Radio.1.OperatingChannelBandwidth`
  - overlay compare after applying the D024 and D025 reruns is now `237 / 420` full matches, `183` mismatches, `62` metadata drifts, actionable workbook-Pass gap `154`, and patch-scope true-open set remains zero
