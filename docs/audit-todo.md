...
- active audit RID is `74ada64b-2026-05-07T134956Z` against local-only workbook `audit/0506.xlsx`; current buckets are `confirmed=151`, `applied=1`, `pending=150`, `block=113`, `needs_pass3=0`
...
- active blockers now include the workbook/audit bucket blockers from this RID plus D006/D009/D012: `113` blocked cases (`103` ambiguous workbook rows, `7` missing workbook rows, `3` source/live authority conflicts); no D004 blocker remains after the reboot/retry closure
- next ready single-case Pass3 target is `D013` (`pass3_pending_no_workbook_extract`); `D005` and `D007` are already in the confirmed bucket (`pass1_verdict_match`)
