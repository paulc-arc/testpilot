...
- `D370 AccessPoint.AssociatedDevice.Active` is now aligned via official rerun `20260415T011605260076`
- workbook authority is row `370`, not stale row `372`; the landed rewrite upgrades the old 5G-only getter into the workbook-faithful tri-band AssociatedDevice live check while keeping the live getter evidence `Active=1/1/1` plus same-MAC `wl assoclist` confirmation on all three bands
...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed aligned case is now `D370 AccessPoint.AssociatedDevice.Active`
- targeted D370/runtime + assocdev guardrails passed; full repo regression is now `1660 passed`; compare is now `343 / 420 full matches` / `77 mismatches` / `57 metadata drifts`, `D355-D357` remain in the CSI-client placeholder bucket, and the next ready non-blocked compare-open case moves to `D371 AccessPoint.AssociatedDevice.DisassociationTime`
