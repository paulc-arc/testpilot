...
- targeted D480/runtime tests, command-budget guardrail, and full repo regression all passed (`1660 passed`); the D481 exploratory rewrite also cleared targeted runtime + command-budget guardrails before the official rerun exposed the 6G drift. Compare therefore still stays at `367 / 420 full matches` / `53 mismatches` while metadata drifts stay at `43`, `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable survey target now moves to `D482 Radio Stats WmmBytesSent AC_BE`
