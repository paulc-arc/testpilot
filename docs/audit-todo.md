...
- `D456 getRadioStats().Noise` is now refreshed via official rerun `20260415T071500138827`
- workbook authority is row `456`; the old row `295` source metadata that still pointed at `WiFi.Radio.{i}.scan()` is now corrected back to the workbook-faithful tri-band `WiFi.Radio.{i}.` / `getRadioStats()` Noise replay
- the rerun exact-closes `Pass / Pass / Pass`: 5G/6G/2.4G return valid integer noise readbacks `-90 / 0 / -78`; final status is `diagnostic_status=Pass`
...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed closure is now `D456 getRadioStats().Noise`
- targeted D456/runtime tests passed; full repo regression remains `1660 passed`; compare stays `362 / 420 full matches` / `58 mismatches` / `47 metadata drifts`, `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable survey target now moves to `D457 getRadioStats().RetransCount`
