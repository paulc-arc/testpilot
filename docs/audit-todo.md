...
- Practical next resume order:
  1. resume the remaining open-set blocker work from `D282`
  2. keep `D324` / `D331` / `D333` blocked unless their drifts are explained with deterministic source-backed corrections
  3. continue `D283-D287` after `D282` is checkpointed, or earlier if new same-source replay evidence appears
