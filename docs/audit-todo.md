...
- `D478 Radio Stats WmmBytesReceived AC_BE` is now aligned via official rerun `20260415T084649232463`
- object/api were already aligned to workbook `WiFi.Radio.{i}.Stats.WmmBytesReceived.` / `AC_BE`, but the old source row `345` plus `getRadioStats() | grep AC_BE_Stats` replay is now retired in favor of workbook row `478` with direct `WiFi.Radio.{i}.Stats.WmmBytesReceived.AC_BE?` getters
- the rerun exact-closes workbook `Pass / Pass / Pass` while keeping `diagnostic_status=Pass`: 5G/6G/2.4G direct getter values all match `wl0/wl1/wl2 wme_counters` `AC_BE` RX bytes at `0 / 0 / 0`
...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed closure is now `D478 Radio Stats WmmBytesReceived AC_BE`
- targeted D478/runtime tests, command-budget guardrail, and full repo regression all passed (`1660 passed`); compare now moves to `365 / 420 full matches` / `55 mismatches` while metadata drifts stay at `43`, `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable survey target now moves to `D479 Radio Stats WmmBytesReceived AC_BK`
