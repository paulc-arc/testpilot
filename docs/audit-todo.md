...
- `D486 Radio Stats WmmFailedBytesReceived AC_BE` is now aligned via official rerun `20260415T102002805516`
- workbook authority is row `486`; the stale row `353` plus `getRadioStats() | grep AC_BE_Stats` replay is retired in favor of direct `WiFi.Radio.{i}.Stats.WmmFailedBytesReceived.AC_BE?` getters plus `wl0/wl1/wl2 wme_counters` `AC_BE` rx failed-byte cross-checks
- a focused serialwrap preprobe earlier saw driver-side `5G=188 / 6G=0 / 2.4G=0` while the direct getters were still zeroed, but the official rerun exact-closes tri-band workbook `Pass / Pass / Pass` at `0 / 0 / 0` with `diagnostic_status=Pass`, so this row does not reproduce a durable blocker
...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed closure is now `D486 Radio Stats WmmFailedBytesReceived AC_BE`
- targeted D486/runtime tests, command-budget guardrail, and full repo regression all passed (`1660 passed`). Compare therefore rises to `370 / 420 full matches` / `50 mismatches` while metadata drifts stay at `43`; `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable survey target now moves to `D487 Radio Stats WmmFailedBytesReceived AC_BK`
