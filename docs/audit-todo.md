...
- `D484 Radio Stats WmmBytesSent AC_VI` is now aligned via official rerun `20260415T100014840516`
- object/api were already aligned to workbook `WiFi.Radio.{i}.Stats.WmmBytesSent.` / `AC_VI`, but the old source row `351` plus `getRadioStats() | grep AC_VI_Stats` replay is now retired in favor of workbook row `484` with direct `WiFi.Radio.{i}.Stats.WmmBytesSent.AC_VI?` getters
- a focused serialwrap preprobe first confirmed tri-band direct getter / driver parity at `0 / 0 / 0`, so this case did not reproduce the 6G zero-getter drift already seen in `D481/D482`
- the rerun exact-closes workbook `Pass / Pass / Pass` while keeping `diagnostic_status=Pass`: 5G/6G/2.4G direct getter values all match `wl0/wl1/wl2 wme_counters` `AC_VI` tx bytes at `0 / 0 / 0`
- focused workbook-faithful rerun `20260415T101223410820` now also marks `D485 Radio Stats WmmBytesSent AC_VO` as a localized blocker: 5G and 2.4G direct getters exact-close `53599 / 43099`, but 6G direct getter `WiFi.Radio.2.Stats.WmmBytesSent.AC_VO?` stays at `0` on both attempts while `wl1 wme_counters` `AC_VO` tx bytes stay at `41612`
...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed closure remains `D484 Radio Stats WmmBytesSent AC_VI`
- targeted D484/runtime tests, command-budget guardrail, and full repo regression all passed (`1660 passed`); the D485 exploratory rewrite also cleared targeted runtime + command-budget guardrails before the official rerun exposed its 6G drift. Compare therefore still stays at `369 / 420 full matches` / `51 mismatches` while metadata drifts stay at `43`, `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable survey target now moves to `D486 Radio Stats WmmFailedBytesReceived AC_BE`
