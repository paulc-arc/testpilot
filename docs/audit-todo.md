...
- active audit RID is `74ada64b-2026-05-07T134956Z` against local-only workbook `audit/0506.xlsx`; current buckets are `confirmed=163`, `applied=8`, `pending=103`, `block=141`, `needs_pass3=0`
- latest blocker is `D060 UplinkMCS`: workbook row 60 expects `Pass / Pass / Pass`, and source declares `AssociatedDevice.UplinkMCS` as a volatile read-only uint32 behind `wld_assocDev_getStats_orf`; focused run `20260509T183228754680` validated the current 5G path with AP1 AssociatedDevice.1 `2C:59:17:00:42:15`, `UplinkMCS=7`, and driver `rx nrate ... mcs 7`, but report shape `Pass / N/A / N/A` mismatches workbook tri-band Pass; adding 6G/2.4G executable coverage is outside the audit allowlist
