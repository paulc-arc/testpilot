...
- latest committed closure is now `D499 SSID WMM AC_VO Stats WmmBytesReceived` via official rerun `20260415T120604251884`
- workbook authority for the landed D499 case is row `499`; the stale row `366` is retired in favor of workbook-faithful direct `WiFi.SSID.{i}.Stats.WmmBytesReceived.AC_VO?` getters plus explicit `WiFi.SSID.{i}.getSSIDStats()` refresh and `wl0/wl1/wl2 wme_counters` `AC_VO` rx-byte cross-checks
- the D499 official rerun exact-closes tri-band workbook `Pass / Pass / Pass` at refresh/direct/driver `45322 / 32323 / 31588` with `diagnostic_status=Pass`
- taken together, `D496` and `D499` now confirm the current compare-open `SSID WmmBytesReceived` family needs an explicit `getSSIDStats()` refresh before the direct getter reread can be judged stably
...
- compare is now `378 / 420 full matches` / `42 mismatches` / `43` metadata drifts
...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed closure is now `D499 SSID WMM AC_VO Stats WmmBytesReceived`
- `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable survey target now moves to `D502 SSID WMM AC_VI Stats WmmBytesSent`
