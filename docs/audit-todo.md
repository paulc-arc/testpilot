...
- Progress record:
  - latest aligned case: `D024 LastDataDownlinkRate` via run `20260412T172957084134`
  - latest blocked case: `none in the D024 closure checkpoint; next ready workbook-Pass case is D025`
  - latest compare summary: `236 / 420` full matches, `184` mismatches, `62` metadata drifts (overlay of full run `20260412T113008433351` plus D024 rerun `20260412T172957084134`)
  ...
  - latest D024 closure: official rerun `20260412T172957084134` exact-closed `LastDataDownlinkRate=487400` against the driver last-tx bucket window `[487400,487500]`; targeted D024 guardrails stayed `6 passed`, full repo regression stayed `1645 passed`, and actionable workbook-Pass gap is now `155`
  ...
  - superseding late-2026-04-12 continuation: recovery commit `338891b7115e5c41d04f45bd79c70ce4b117cebc` has already been pushed, authoritative full run `20260412T113008433351` has already completed cleanly, and the post-relaunch focus has now moved on to stale-row closure rather than baseline recovery
  ...
  - `D025` remains the next ready stale-row case: workbook authority is row `25`, current YAML metadata is still stale at row `22`, and the latest overlay compare still leaves only the projected 6G band mismatched (`Pass / Fail / Pass` vs workbook `Pass / Pass / Pass`)
  - `D026` remains the next stale-row follow-up after D025: workbook authority is row `26`, and the source evidence still points to DUT `wl -i wl0 sta_info $STA_MAC` `link bandwidth = XX MHZ` plus `WiFi.Radio.1.OperatingChannelBandwidth`
  - overlay compare after applying the D024 rerun is now `236 / 420` full matches, `184` mismatches, `62` metadata drifts, actionable workbook-Pass gap `155`, and patch-scope true-open set remains zero
