...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed closure is now `D484 Radio Stats WmmBytesSent AC_VI`
- targeted D484/runtime tests, command-budget guardrail, and full repo regression all passed (`1660 passed`); the D481 and D482 exploratory rewrites also cleared targeted runtime + command-budget guardrails before the official reruns exposed their 6G drifts. Compare therefore now moves to `369 / 420 full matches` / `51 mismatches` while metadata drifts stay at `43`, `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable survey target now moves to `D485 Radio Stats WmmBytesSent AC_VO`
