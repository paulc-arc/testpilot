...
- active audit RID is `74ada64b-2026-05-07T134956Z` against local-only workbook `audit/0506.xlsx`; current buckets are `confirmed=160`, `applied=8`, `pending=112`, `block=135`, `needs_pass3=0`
...
- next ready single-case Pass3 target is `D052` (`pass3_pending_source_survey_required`); `D005`, `D007`, `D010`, `D011`, `D023`, `D029`, `D036`, `D038`, `D040`, `D042`, `D048`, `D049`, `D050`, and pass1-confirmed `D027` are in the confirmed bucket
- latest D051 validation: focused `testpilot wifi_llapi --case D051 --dut-fw-ver BGW720-0410-VERIFY` -> `0 passed / 1 failed`, `Fail / N/A / N/A`, compare `full_match_count=0` / `mismatch_case_count=1` against workbook row 51 `Pass / Pass / Pass`; blocker is stale case-local WPA3/SAE `sta_env_setup[48]` plus checked-in 5G-only scope, both outside audit allowlist
