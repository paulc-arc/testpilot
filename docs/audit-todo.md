...
- active audit RID is `74ada64b-2026-05-07T134956Z` against local-only workbook `audit/0506.xlsx`; current buckets are `confirmed=151`, `applied=4`, `pending=142`, `block=118`, `needs_pass3=0`
- latest applied/pass3 closure is `D020 FrequencyCapabilities` via focused run `20260509T133802129550`: the final single-case rerun reports `Pass / Pass / Pass`, `diagnostic_status=Pass`, and `pass_count=1 / fail_count=0`; compare against `audit/0506.xlsx` reports `full_match_count=1`, `mismatch_case_count=0` for D020
