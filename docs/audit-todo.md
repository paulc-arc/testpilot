...
- latest committed closure is now `D502 SSID WMM AC_VI Stats WmmBytesSent` via official rerun `20260415T121734070494`
- workbook authority for the landed D502 case is row `502`; the stale row `369` is retired in favor of workbook-faithful direct `WiFi.SSID.{i}.Stats.WmmBytesSent.AC_VI?` getters plus explicit `WiFi.SSID.{i}.getSSIDStats()` refresh and `wl0/wl1/wl2 wme_counters` `AC_VI` tx-byte cross-checks
- the D502 official rerun exact-closes tri-band workbook `Pass / Pass / Pass` at refresh/direct/driver `0 / 0 / 0` with `diagnostic_status=Pass`
- taken together, `D496`, `D499`, and `D502` now confirm the current compare-open SSID-level WMM stats family needs an explicit `getSSIDStats()` refresh before the direct getter reread can be judged stably
...
- compare is now `379 / 420 full matches` / `41 mismatches` / `43` metadata drifts
...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed closure is now `D502 SSID WMM AC_VI Stats WmmBytesSent`
- `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable survey target now moves to `D505 SSID WMM AC_BK Stats WmmFailedBytesReceived`
