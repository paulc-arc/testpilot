...
- latest committed closure is now `D492 Radio Stats WmmFailedBytesSent AC_VI` via official rerun `20260415T112139068980`
- workbook authority for the landed D492 case is row `492`; the stale row `359` plus `getRadioStats() | grep AC_VI_Stats` replay is retired in favor of lowercase direct `WiFi.Radio.{i}.Stats.WmmFailedbytesSent.AC_VI?` getters plus `wl0/wl1/wl2 wme_counters` `AC_VI` tx failed-byte cross-checks
- the D492 official rerun exact-closes tri-band workbook `Pass / Pass / Pass` at `0 / 0 / 0` with `diagnostic_status=Pass`
- the D492 focused survey and official rerun both confirm the same clean family shape: direct getters stay `0 / 0 / 0`, driver `AC_VI` tx failed bytes also stay `0 / 0 / 0`, and the startup `serialwrap daemon start failed` warning still does not invalidate the saved decoded logs
- the immediately preceding `D491 Radio Stats WmmFailedBytesSent AC_BK` closure remains the same tri-band `0 / 0 / 0` exact-close
...
- compare is now `375 / 420 full matches` / `45 mismatches` / `43` metadata drifts
...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed closure is now `D492 Radio Stats WmmFailedBytesSent AC_VI`
- `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable survey target now moves to `D493 Radio Stats WmmFailedBytesSent AC_VO`
