...
- active audit RID is `74ada64b-2026-05-07T134956Z` against local-only workbook `audit/0506.xlsx`; current buckets are `confirmed=163`, `applied=8`, `pending=102`, `block=142`, `needs_pass3=0`
- latest blocker is `D061 UplinkShortGuard`: workbook row 61 expects `Pass / Pass / Pass`, and source declares `AssociatedDevice.UplinkShortGuard` as a volatile read-only bool behind `wld_assocDev_getStats_orf`; focused run `20260509T183612306374` validated the current 5G path with AP1 AssociatedDevice.1 `2C:59:17:00:42:15`, `UplinkShortGuard=1`, and driver `rx nrate ... GI 1.6us` mapping to `DriverUplinkShortGuard=1`, but report shape `Pass / N/A / N/A` mismatches workbook tri-band Pass; adding 6G/2.4G executable coverage is outside the audit allowlist
