...
- latest committed closure is now `D523 SSID WMM AC_VO Stats WmmPacketsReceived` via official rerun `20260415T154857895725`
...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed closure is now `D523 SSID WMM AC_VO Stats WmmPacketsReceived`
- `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable survey target now moves to `D524 SSID WMM AC_BE Stats WmmPacketsSent`
