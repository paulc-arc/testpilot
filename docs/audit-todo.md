...
- Latest operating-standards blocker refinement:
  - `D282 getScanResults() OperatingStandards` remains blocked with stronger source/runtime confidence
  - 0403 source now proves `WiFi.NeighboringWiFiDiagnostic()` is not replaying cached `lastScanResults`; it launches fresh `SCAN_TYPE_INTERNAL` scans on every radio and only serializes data from radios that enter `completedRads` (`ReportingRadios`), while start failures or unsuccessful scan-complete callbacks land in `FailedRadios`
  - same-window live probe now shows the blocker is not just stale ambient data: `getScanResults()` still returns first targets for all three radios (`5G=2C:59:17:00:03:E5`, `6G=2C:59:17:00:19:96`, `2.4G=2C:59:17:00:03:F7`), but immediately repeated `NeighboringWiFiDiagnostic()` still returns only `ReportingRadios="wl0"` / `FailedRadios="wl1,wl2"` and only includes the 5G target
  - the focused recovery experiment `wifi-llapi baseline-qualify --band 6g --band 2.4g --repeat-count 1 --soak-minutes 0` also failed to make the sibling oracle durable: 2.4G qualification became stable, 6G still failed post-verify on `dut_ocv_not_zero`, and even right after that recovery `Device.WiFi.Radio.2/3.ChannelMgt.RadioStatus` still read `Down`
  - therefore there is still no all-band same-source sibling replay that is commit-worthy for row `282`, and there is currently no new ready open-set case beyond blocked `D281` / `D282`
