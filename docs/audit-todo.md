...
- latest committed closure remains `D489 Radio Stats WmmFailedBytesReceived AC_VO` via official rerun `20260415T105002687631`
- workbook authority for the landed D489 case is row `489`; the stale row `356` plus `getRadioStats() | grep AC_VO_Stats` replay is retired in favor of direct `WiFi.Radio.{i}.Stats.WmmFailedBytesReceived.AC_VO?` getters plus `wl0/wl1/wl2 wme_counters` `AC_VO` rx failed-byte cross-checks
- the D489 official rerun exact-closes tri-band workbook `Pass / Pass / Pass` at `0 / 0 / 0` with `diagnostic_status=Pass`; the focused preprobe also confirms this is not another D481-style mismatch because AC_VO traffic bytes are non-zero while the authoritative `failed bytes` field still stays at zero on all three bands
- `D486` / `D487` / `D488` remain the immediately preceding aligned closures, all exact-closing tri-band `0 / 0 / 0`
- focused official rerun `20260415T110106382425` now parks `D490 Radio Stats WmmFailedBytesSent AC_BE` as a new localized blocker instead of a closure
...
- that makes D490 another localized 6G zero-getter blocker; the exploratory workbook-faithful rewrite was rolled back instead of being committed, so compare stays unchanged at `373 / 420 full matches` / `47 mismatches` / `43` metadata drifts
- the D490 official rerun emitted the same `serialwrap daemon start failed` startup warning seen in some earlier focused runs, but decoded DUT/STA logs were still saved and the blocker evidence remains usable
...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed closure remains `D489 Radio Stats WmmFailedBytesReceived AC_VO`
- `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable survey target now moves to `D491 Radio Stats WmmFailedBytesSent AC_BK`
